...
GLuint CreateMyWindow(LPSTR strWindowName, GLint width, GLint height, DWORD dwStyle, GLboolean bFullScreen, HINSTANCE hInstance)
{
	...
	wndclass.hIcon =LoadIcon(hInstance, MAKEINTRESOURCE(IDI_ICON1));	// General icon
...
{  
	...
	SizeOpenGLScreen(width, height);					// Setup the screen translatioport
CreateTexture(g_Texture,"Data/Damage.bmp",2);
CreateTexture(g_Texture,"Data/Explo.bmp",3);
CreateTexture(g_Texture,"Data/Win.bmp",1);
}
...
{	
	...
	hWnd = CreateMyWindow("Collisions in 2D world", SCREEN_WIDTH, SCREEN_HEIGHT, 0, g_bFullScreen, hInstance);
