...
public:
	union
	{
		struct {GLfloat x, y;}; // Data used for position on axises
		GLfloat v[2];
	};
...
public:
	union
	{
		struct {GLfloat x, y, z;}; // Data used for position on axises
		GLfloat v[3];
	};
...
public:
	...
	GLvoid changeDisplaying(GLint scrWidth, GLint scrHeight, GLint bpp, GLint iFS);
	GLvoid switchFS();
