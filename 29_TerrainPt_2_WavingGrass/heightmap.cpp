...
CShaderProgram CMultiLayeredHeightmap::spGrass;
...
GLboolean CMultiLayeredHeightmap::LoadTerrainShaderProgram()
{
	...
	bOK &= shShaders[3].LoadShader("data\\shaders\\grass.vert", GL_VERTEX_SHADER);
	bOK &= shShaders[4].LoadShader("data\\shaders\\grass.geom", GL_GEOMETRY_SHADER);
	bOK &= shShaders[5].LoadShader("data\\shaders\\grass.frag", GL_FRAGMENT_SHADER);

	spTerrain.CreateProgram();
	FOR(i, 3)spTerrain.AddShaderToProgram(&shShaders[i]);
...
GLvoid CMultiLayeredHeightmap::RenderHeightmap()
{
	...
	spGrass.SetUniform("fTimePassed", fTimePassed);
...
CShaderProgram* CMultiLayeredHeightmap::GetGrassShaderProgram()
{
	return &spGrass;
}
