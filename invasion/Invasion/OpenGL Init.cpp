...
void SizeOpenGLScreen(int width, int height)			// Initialize The GL Window
{
	...
	gluPerspective(45.0f,(float)width/(float)height, 1 ,1000.0f);
...
{
	g_DC=GetDC(g_Wnd); // Set global device context
	if(!SetupPixelFormat(g_DC)) // Setup Pixel format
	{
		PostQuitMessage(0);
	}

	g_RC=wglCreateContext(g_DC); // Create rendering context
	wglMakeCurrent(g_DC,g_RC);   // Set the rendering context
