...
void main()
{
    ...
    for(int i = 0; i < 3; i++)vNormalPass[i] = (vec4(vNormalPass[i], 1.0)*matrices.normalMatrix).xyz;
    vec3 vNormalMiddle = (vNormalPass[0]+vNormalPass[1]+vNormalPass[2])/3.0;
    for(int i = 0; i < 3; i++)
    {
      vec3 vPos = gl_in[i].gl_Position.xyz;
      gl_Position = mMVP*vec4(vPos, 1.0);
      vNormal = (vec4(vNormalPass[i], 1.0)).xyz;
      vTexCoord = vTexCoordPass[i];
      EmitVertex();

      vPos = gl_in[(i+1)%3].gl_Position.xyz;
      gl_Position = mMVP*vec4(vPos, 1.0);
      vNormal = (vec4(vNormalPass[(i+1)%3], 1.0)).xyz;
